...
def calculate_metrics(predicted_labels, ground_truth_labels, all_labels):
    ...
    for label in all_labels:
        ...
        unique_predicted = sum(
            1 for p, g in zip(predicted, ground_truth) if p == 1 and g == 0
        )
        unique_ground_truth = sum(
            1 for p, g in zip(predicted, ground_truth) if p == 0 and g == 1
        )
    ...
    total_unique_predicted = sum(
        scores[label]["unique_predicted"] for label in all_labels
    )
    total_unique_ground_truth = sum(
        scores[label]["unique_ground_truth"] for label in all_labels
    )
...
def visualize_emotion_data(scores, save, path, percentage=False, run_numbers=None):
        ...
        if emotion != "average":  # Skip the average key
            ...
            if percentage and total > 0:
                ...
                emotion_data[emotion] = [
                    unique_ground_truth_pct,
                    matching_pct,
                    unique_predicted_pct,
                ]
            else:
                emotion_data[emotion] = [
                    unique_ground_truth,
    ...
    df_prediction_matching = pd.DataFrame.from_dict(
        ...
        orient="index",
        columns=[
            f"Unique Run {run_numbers[0]}",
            "Matching",
            f"Unique Run {run_numbers[1]}",
        ],
    )
    ...
    sns.barplot(x=df_recall.index, y="Recall", data=df_recall, ax=axes[0, 1])
...
def emotion_analysis_comparison_pipeline(
    run_number1, run_number2, set_, EMOTIONS, plot_percentage=False, save=False
):
    ...
    visualize_emotion_data(
        scores,
        save,
        f"plots/emotion_metrics_run{run_number1}_vs_run{run_number2}",
        plot_percentage,
        run_numbers=[run_number1, run_number2],
    )
    ...
    all_labels = sorted(
        set(list(label_counts_run1.keys()) + list(label_counts_run2.keys()))
    )
    ...
    if save:
        plt.savefig(
            f"plots/label_distribution_run{run_number1}_vs_run{run_number2}.png"
        )
    plt.show()
    ...
    ax1.bar(
        [f"Run {run_number1}", f"Run {run_number2}"],
        [run1_values[0], run2_values[0]],
        color=["blue", "red"],
    )
    ...
    plt.suptitle(
        f"Diversity Measures Comparison (Run {run_number1} vs Run {run_number2})"
    )
    plt.tight_layout()

    if save:
        plt.savefig(
            f"plots/diversity_measures_run{run_number1}_vs_run{run_number2}.png"
        )
    ...
    print(
        f"Average number of labels per comment - Run {run_number1}: {average_labels_run1:.4f}"
    )
    print(
        f"Average number of labels per comment - Run {run_number2}: {average_labels_run2:.4f}"
    )


def emotion_pair_analysis_pipeline(
    run_number, set_, config_path, number=10, table=False, save_visualizations=False
):
    ...
    def plot_labels(data, title, run_number, save=False, set_="train"):
    ...
    def visualize_data(df, emotions, save=False):
        ...
        axs[0].barh(
            list(sorted_filtered_dict.keys()), list(sorted_filtered_dict.values())
        )
    ...
    # Get emotion pairs
    top_labels = get_top_labels(data, n=number)
    top_pairs = get_top_pairs(data, emotions)

    # Plot and save
    plot_labels(
        top_labels,
        f"Most {number} Common labels",
        run_number,
        save=True,
        set_=set_,
    )
    plot_labels(
        top_pairs,
        f"Most common {number} Emotion Pairs",
        run_number,
        save=True,
        set_=set_,
    )
    plot_heatmap(
        get_top_pairs(data, emotions, n=len(emotions) ** 2),
        emotions,
        run_number,
        save=True,
        set_=set_,
    ...
    if table:
        print_table(top_labels, f"Most {number} Common labels")
        print_table(top_pairs, f"Most common {number} Emotion Pairs")
