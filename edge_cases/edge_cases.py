...
our_model = AutoModelForSequenceClassification.from_pretrained("0ssamaak0/roberta-base-LEGO_emotions")

original_tokenizer = AutoTokenizer.from_pretrained("SamLowe/roberta-base-go_emotions")
original_model = AutoModelForSequenceClassification.from_pretrained("SamLowe/roberta-base-go_emotions")
...
def analyze_emotion(text, tokenizer, model, emotions_list):
    ...
    sorted_emotions = sorted(zip(emotions_list, probabilities), key=lambda x: x[1], reverse=True)

    df = pd.DataFrame(sorted_emotions[:5], columns=['Emotion', 'Score'])  # Limit to top 5 emotions
...
def save_edge_case(text, our_df, original_df, save_our, save_original):
    ...
    with open(csv_filename, 'a', newline='') as csvfile:
        ...
        fieldnames += [f"emotion_1_{i+1}" for i in range(5)] + [f"probability_1_{i+1}" for i in range(5)]
        fieldnames += [f"emotion_2_{i+1}" for i in range(5)] + [f"probability_2_{i+1}" for i in range(5)]
...
with gr.Blocks() as iface:
    ...
    gr.Markdown("Analyze the emotions in the given text with two models and save interesting edge cases.")
    ...
    with gr.Row():
        ...
        save_original = gr.Checkbox(label="Save for Original Model (Model 2)", value=False)
    ...
    def analyze(text):
        ...
        original_df = analyze_emotion(text, original_tokenizer, original_model, original_EMOTIONS)
    ...
    analyze_btn.click(analyze, inputs=text_input, outputs=[output_our_df, output_original_df, last_analyzed_text, last_our_df, last_original_df])
    save_btn.click(save, inputs=[last_analyzed_text, last_our_df, last_original_df, save_our, save_original], outputs=save_status)
